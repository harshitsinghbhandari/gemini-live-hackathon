...
def listen_to_audit_log(callback):
    ...
    col_query = db.collection("audit_log").order_by("timestamp", direction=firestore.Query.DESCENDING).limit(1)
    # The watch functionality in the python SDK is synchronous/blocking in its own thread
    # and doesn't have an officially supported async version for collection watches yet.
    # However, it works by spawning a thread.
    # For now, keeping the watch but ensuring the callback is handled correctly.
